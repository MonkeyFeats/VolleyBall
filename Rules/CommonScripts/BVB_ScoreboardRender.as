...
float drawScoreboard(CPlayer@[] players, Vec2f topleft, CTeam@ team, Vec2f emblem)
{
	...
	f32 stepheight = 16;
	Vec2f bottomright(getScreenWidth() - 364, topleft.y + (players.length + 5.5) * stepheight);
	GUI::DrawPane(topleft, bottomright, SColor(100, team.color.getRed(), team.color.getGreen(), team.color.getBlue()));
	...
	GUI::DrawText(getTranslatedString("Players: {PLAYERCOUNT}").replace("{PLAYERCOUNT}", "" + players.length), Vec2f(bottomright.x - 300, topleft.y), SColor(0xffffffff));

	topleft.y += stepheight * 2;

	//draw player table header
	GUI::DrawText(getTranslatedString("Player"), Vec2f(topleft.x, topleft.y), SColor(0xffffffff));
	GUI::DrawText(getTranslatedString("Username"), Vec2f(bottomright.x - 300, topleft.y), SColor(0xffffffff));
	GUI::DrawText(getTranslatedString("Ping"), Vec2f(bottomright.x - 64, topleft.y), SColor(0xffffffff));
	...
	for (u32 i = 0; i < players.length; i++)
	{
		...
		bool playerHover = mousePos.y > topleft.y && mousePos.y < topleft.y + 15 && mousePos.x > topleft.x && mousePos.x < bottomright.x;
		...
		GUI::DrawText("" + username, Vec2f(bottomright.x - 300, topleft.y), namecolour);
		GUI::DrawText("" + ping_in_ms, Vec2f(bottomright.x - 64, topleft.y), SColor(0xffffffff));
...
void onRenderScoreboard(CRules@ this)
{
	...
	if (spectators.length > 0) // need to fix the step height vs text dim stuff
	{
		//draw spectators
		f32 stepheight = 16;
		f32 extra = (spectators.length % 5 == 0 ? 0 : 32);
		Vec2f bottomright(getScreenWidth() - 364, topleft.y + extra + (spectators.length/5)*stepheight * 2);
		f32 specy = topleft.y + stepheight * 0.5;
		GUI::DrawPane(topleft, bottomright, SColor(0x4dc0c0c0));
		...
		{			
			...
			CPlayer@ p = spectators[i];
			{
				...
				SColor namecolour = getNameColour(p);
				GUI::DrawText(name, Vec2f(topleft.x+ 100 +specx, specy-32), namecolour);
				specx += textdim.x + 10;
			}
		}
	}
