...
void onTick(CBlob@ this)
{
	...
	if (this.isKeyJustPressed(key_pickup))
	{
		...
		Vec2f vec = aimpos - this.getPosition();
		f32 angle = vec.Angle();

		if (carryBlob !is null && carryBlob.getName() == "beachball")
		{
		...
		else if (this.isAttached()  && carryBlob.getName() != "beachball") // default drop from attachment
		{
		...
		else if (carryBlob !is null && !carryBlob.hasTag("custom drop") && (!carryBlob.hasTag("temp blob") || carryBlob.getName() == "ladder") && carryBlob.getName() != "beachball")
		{
			ClearPickupBlobs(this);
			client_SendThrowCommand(this);
			this.set_bool("release click", false);
